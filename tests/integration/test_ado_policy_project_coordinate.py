...
def test_policy_status_uses_valid_ado_project_coordinate_and_cache(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Status fetches and caches ``apm/_apm/apm-policy.yml`` for ADO."""
    ...
    assert report["source"] == "org:dev.azure.com/contoso/apm/_apm"
    assert request.call_count == 1

    request_url = urlparse(request.call_args.args[0])
    assert request_url.hostname == "dev.azure.com"
    assert unquote(request_url.path) == "/contoso/apm/_apis/git/repositories/_apm/items"
    ...
    cache_ref = "dev.azure.com/contoso/apm/_apm"
