...
- CachedDependencySource.acquire:
  ...
  - without a supported manifest (fail closed)
...
class TestLocalDependencySourceUserScope:
    ...
    def test_accepts_absolute_path_at_user_scope(self, tmp_path):
        ...
        src.mkdir()
        manifest = "name: test\nversion: 1.0.0\ndependencies:\n  apm:\n    - owner/repo\n"
        (src / "apm.yml").write_text(manifest)
        install_path = tmp_path / "apm_modules" / "pkg"
        install_path.mkdir(parents=True)
        (install_path / "apm.yml").write_text(manifest)
...
# ---------------------------------------------------------------------------
# LocalDependencySource.acquire -- fail closed without a supported manifest
# ---------------------------------------------------------------------------


class TestLocalDependencySourceNoApmYml:
    def test_rejects_package_without_supported_manifest(self, tmp_path):
...
class TestLocalDependencySourceMarketplacePlugin:
    def test_legacy_plugin_still_normalizes_through_validation(self, tmp_path):
        from apm_cli.install.sources import LocalDependencySource
        from apm_cli.models.apm_package import PackageType
...
def _make_local_pkg_with_symlink(root: Path) -> Path:
    ...
    (instructions / "test.instructions.md").write_text("# Test\n")
