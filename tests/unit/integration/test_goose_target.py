...
def test_goose_accepted_by_resolve_targets(tmp_path: Path) -> None:
    # Regression: the MCP-install phase resolves `--target` via resolve_targets,
    # which used to validate against the GA-only CANONICAL_TARGETS and crashed
    # `apm install --target goose` whenever the package declared an mcp: dep.
    from apm_cli.core.target_detection import resolve_targets

    resolved = resolve_targets(tmp_path, flag="goose")
    assert resolved.targets == ["goose"]


def test_goose_runtime_discovered_when_opted_in(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    # The MCP runtime-discovery list must include goose (flag on + config dir
    # present), else `apm install --target goose` deploys recipe/skill but
    # silently skips writing the MCP server to ~/.config/goose/config.yaml.
    from apm_cli.integration import mcp_integrator_install as mii

    monkeypatch.setenv("XDG_CONFIG_HOME", str(tmp_path))
    (tmp_path / "goose").mkdir()

    monkeypatch.setattr("apm_cli.core.experimental.is_enabled", lambda name: name == "goose")
    assert mii._goose_runtime_opted_in() is True

    monkeypatch.setattr("apm_cli.core.experimental.is_enabled", lambda name: False)
    assert mii._goose_runtime_opted_in() is False
...
def test_goose_transform_fails_closed_on_non_mapping() -> None:
    with pytest.raises(ValueError):
        GooseClientAdapter()._to_native_format("bad", "not-a-dict")  # type: ignore[arg-type]


def test_goose_transform_drops_malformed_typed_fields() -> None:
    # Non-dict headers/envs and non-list args must be dropped, never crash.
    remote = GooseClientAdapter()._to_native_format(
        "r", {"type": "http", "url": "https://x", "headers": "oops"}
    )
    assert "headers" not in remote
    stdio = GooseClientAdapter()._to_native_format(
        "s", {"command": "npx", "args": "oops", "env": "oops"}
    )
    assert "args" not in stdio and "envs" not in stdio


def test_goose_update_config_fails_closed_on_bad_entry(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    # A scalar server config makes the transform raise; update_config must
    # return False (not crash) and write nothing.
    adapter = _adapter_with_home(monkeypatch, tmp_path)
    assert adapter.update_config({"bad": "not-a-dict"}) is False
    assert not Path(adapter.get_config_path()).exists()
...
def test_goose_detect_target_echoes_explicit_and_config(tmp_path: Path) -> None:
    # Regression: detect_target must round-trip an explicit/config ``goose``
    # rather than falling back to "minimal" -- otherwise `apm compile -t goose`
    # silently drops the .goosehints stub (it only emits AGENTS.md).
    from apm_cli.core.target_detection import detect_target

    explicit, _ = detect_target(tmp_path, explicit_target="goose")
    assert explicit == "goose"
    assert should_compile_goose_hints(explicit) is True

    config, _ = detect_target(tmp_path, config_target="goose")
    assert config == "goose"
...
def test_goose_agent_compiles_to_recipe_yaml(tmp_path: Path) -> None:
    ...
    # Headless-ready: a prompt is always present (generic default here).
    assert recipe["prompt"]
    # MCP extensions are NOT embedded (agents declare no MCP servers).
    assert "extensions" not in recipe


def test_goose_recipe_uses_authored_prompt_verbatim(tmp_path: Path) -> None:
    package_dir = tmp_path / "pkg"
    agents_dir = package_dir / ".apm" / "agents"
    agents_dir.mkdir(parents=True)
    (agents_dir / "analyzer.agent.md").write_text(
    ...
    recipe = _yaml.safe_load(
        (tmp_path / ".goose" / "recipes" / "analyzer.yaml").read_text(encoding="utf-8")
    )
    assert recipe["prompt"] == "Analyze {{url}} end-to-end."


def test_goose_recipe_passes_parameters_and_preserves_templating(tmp_path: Path) -> None:
    package_dir = tmp_path / "pkg"
    agents_dir = package_dir / ".apm" / "agents"
    agents_dir.mkdir(parents=True)
    (agents_dir / "crawler.agent.md").write_text(
        "---\n"
        "name: crawler\n"
    ...
    raw = (tmp_path / ".goose" / "recipes" / "crawler.yaml").read_text(encoding="utf-8")
    # {{ }} placeholders survive verbatim in prompt and instructions.
    assert "{{ url }}" in raw

    import yaml as _yaml

    recipe = _yaml.safe_load(raw)
    # parameters block passes through verbatim (Goose schema).
    ...
    assert "{{ url }}" in recipe["prompt"]
    assert "{{ url }}" in recipe["instructions"]


def test_goose_recipe_omits_parameters_when_none(tmp_path: Path) -> None:
    package_dir = tmp_path / "pkg"
    agents_dir = package_dir / ".apm" / "agents"
    agents_dir.mkdir(parents=True)
    ...
    recipe = _yaml.safe_load(
        (tmp_path / ".goose" / "recipes" / "plain.yaml").read_text(encoding="utf-8")
    )
    assert "parameters" not in recipe


def test_goose_recipe_ignores_non_string_name_and_model(tmp_path: Path) -> None:
    package_dir = tmp_path / "pkg"
    agents_dir = package_dir / ".apm" / "agents"
    agents_dir.mkdir(parents=True)
    # Malformed frontmatter: name/model as lists must not propagate.
    (agents_dir / "weird.agent.md").write_text(
        "---\nname:\n  - a\n  - b\ndescription: ok\nmodel:\n  - x\n---\n\nBody.\n",
        encoding="utf-8",
    )
    ...
    # Filename stem is used because `name` was not a string.
    recipe = _yaml.safe_load(
        (tmp_path / ".goose" / "recipes" / "weird.yaml").read_text(encoding="utf-8")
    )
    assert recipe["title"] == "weird"
    assert "settings" not in recipe  # non-string model dropped


def test_goose_recipe_falls_back_to_default_prompt(tmp_path: Path) -> None:
    package_dir = tmp_path / "pkg"
    agents_dir = package_dir / ".apm" / "agents"
    agents_dir.mkdir(parents=True)
    (agents_dir / "persona.agent.md").write_text(
        "---\nname: persona\ndescription: d\n---\n\nYou are a persona.\n", encoding="utf-8"
    )
    ...
    recipe = _yaml.safe_load(
        (tmp_path / ".goose" / "recipes" / "persona.yaml").read_text(encoding="utf-8")
    )
    assert recipe["prompt"] == AgentIntegrator._DEFAULT_RECIPE_PROMPT


def test_goose_recipe_renders_multiline_instructions_as_block(tmp_path: Path) -> None:
    package_dir = tmp_path / "pkg"
    agents_dir = package_dir / ".apm" / "agents"
    agents_dir.mkdir(parents=True)
    body = "You are a reviewer.\n\n## Step 1\nDo the thing.\n\n## Step 2\n- a\n- b"
    (agents_dir / "rev.agent.md").write_text(
        f"---\nname: rev\ndescription: d\n---\n\n{body}\n", encoding="utf-8"
    )
    ...
    raw = (tmp_path / ".goose" / "recipes" / "rev.yaml").read_text(encoding="utf-8")
    # Readable literal block scalar, not a quoted flow scalar.
    assert "instructions: |" in raw
    assert "instructions: '" not in raw

    import yaml as _yaml

    # ...and the text still round-trips exactly.
    assert _yaml.safe_load(raw)["instructions"] == body
