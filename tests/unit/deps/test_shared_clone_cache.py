...
from apm_cli.deps.shared_clone_cache import SharedCloneCache, _RepoCoords
...
class TestSharedCloneCache:
    ...
    def test_single_subdir_dep_clones_once(self, tmp_path: Path) -> None:
        ...
        result = cache.get_or_clone(
            _RepoCoords(host="github.com", owner="owner", repo="repo"), "main", clone_fn
        )
    ...
    def test_divergent_refs_clone_independently(self, tmp_path: Path) -> None:
        ...
        path1 = cache.get_or_clone(
            _RepoCoords(host="github.com", owner="owner", repo="repo"), "v1.0", clone_fn
        )
        path2 = cache.get_or_clone(
            _RepoCoords(host="github.com", owner="owner", repo="repo"), "v2.0", clone_fn
        )
    ...
    def test_failure_surfaces_to_all_consumers(self, tmp_path: Path) -> None:
        ...
        with pytest.raises(RuntimeError, match="network timeout"):
            cache.get_or_clone(
                _RepoCoords(host="github.com", owner="owner", repo="repo"), "main", failing_clone
            )
    ...
    def test_concurrent_access_serializes_clone(self, tmp_path: Path) -> None:
        ...
        def worker() -> None:
            try:
                p = cache.get_or_clone(
                    _RepoCoords(host="github.com", owner="owner", repo="repo"), "main", slow_clone
                )
                results.append(p)
    ...
    def test_context_manager_cleanup(self, tmp_path: Path) -> None:
        ...
        with SharedCloneCache(base_dir=tmp_path) as cache:
            ...
            path = cache.get_or_clone(
                _RepoCoords(host="github.com", owner="o", repo="r"), None, clone_fn
            )
            assert path.exists()
...
class TestBareCacheRaceCondition:
    ...
    def test_parallel_different_subdirs_both_succeed(self, tmp_path: Path) -> None:
        ...
        def thread_a() -> None:
            try:
                bare = cache.get_or_clone(
                    _RepoCoords(host="h", owner="o", repo="r"), "main", populate_bare
                )
...
class TestMaterializeFromBare:
    ...
    def test_apm_debug_rejects_non_bare_clone(self, tmp_path: Path, monkeypatch) -> None:
        ...
        with pytest.raises(RuntimeError, match="not a bare repo"):
            cache.get_or_clone(_RepoCoords(host="h", owner="o", repo="r"), "main", bad_populate)
        cache.cleanup()
    ...
    def test_apm_debug_accepts_bare_clone(self, tmp_path: Path, monkeypatch) -> None:
        ...
        path = cache.get_or_clone(_RepoCoords(host="h", owner="o", repo="r"), "main", good_populate)
...
class TestSharedCloneCacheRepoReuse:
    ...
    def test_fetch_fn_reuses_existing_bare_for_different_sha(self, tmp_path: Path) -> None:
        ...
        path1 = cache.get_or_clone(
            _RepoCoords(host="github.com", owner="owner", repo="repo"), "main", clone_fn_1
        )

        # Second call: fetch_fn should be tried for the SHA
        sha = "a" * 40
        path2 = cache.get_or_clone(
            _RepoCoords(host="github.com", owner="owner", repo="repo"),
            sha,
            clone_fn_2,
    ...
    def test_fetch_fn_none_skips_tier0(self, tmp_path: Path) -> None:
        ...
        cache.get_or_clone(_RepoCoords(host="gh", owner="o", repo="r"), "main", clone_fn)
        ...
        cache.get_or_clone(
            _RepoCoords(host="gh", owner="o", repo="r"), "dev", clone_fn2, fetch_fn=None
        )
