...
COMMENT_SKILL = ROOT / "packages/autopilot/autopilot-comment/SKILL.md"
...
def test_triage_workers_default_debug_off_posts_suggested_comment() -> None:
    """Public GitHub writes are the suggested comment unless debug is on."""
    for path in (TRIAGE_SKILL, WORKER_PR_TRIAGE, COMMENT_SKILL):
        text = _ascii(path)
        assert "debug: off | on" in text, path
        assert "`debug` defaults to `off` when omitted or unknown" in text, path
        assert "Omitted `debug` is not a missing-field stop" in text, path
        assert "`[i] Skill debug is on.`" in text, path
    issue = _ascii(TRIAGE_SKILL)
    assert "Suggested issue comment" in issue
    assert "autopilot-comment" in issue
    pr = _ascii(WORKER_PR_TRIAGE)
    assert "Suggested PR comment" in pr
    assert "autopilot-comment" in pr
    workflow = _ascii(TRIAGE_WORKFLOW)
    assert "`debug: off`" in workflow
    assert "autopilot-comment" in workflow
    comment = _ascii(COMMENT_SKILL)
    assert "Generated by <source_skill>. This comment is AI-generated and may contain errors." in (
        comment
    )


def test_comment_writers_load_autopilot_comment() -> None:
    """Skills that post GitHub comments delegate the write to autopilot-comment."""
    for path in (TRIAGE_SKILL, WORKER_PR_TRIAGE, REVIEW_SKILL, WORKER_PR_SKILL):
        text = _ascii(path)
        assert "autopilot-comment" in text, path
        assert "source_skill:" in text, path
...
def test_implementation_harnesses_assign_issue_and_pr_not_reviewer() -> None:
    ...
    assert "autopilot-comment" in pr_worker
