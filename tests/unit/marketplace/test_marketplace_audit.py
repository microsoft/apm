...
class TestClassifyDependency:
    ...
    def test_empty(self, dep):
        ...
        # normal schema validator (not audit) surface the underlying issue.
...
class TestCollectApmDepStrings:
    ...
    def test_dict_form_git_entry_is_captured(self):
        ...
        collection would silently drop ``{git: ...}`` object-style
        entries, which must be flagged just like bare URL strings.
        """
        data = {
            "dependencies": {
                "apm": [
...
class TestResolvePluginGithubCoords:
    ...
    def test_repo_with_extra_slashes_unsupported(self):
        # A misconfigured ``repo: 'org/repo/extra'`` would otherwise be
        # silently coerced to owner='org', name='repo/extra' -- yielding
        # a malformed GitHub URL that 404s and gets reported as
        # NO_MANIFEST (skipped).  Reject it at the resolver so audit
        # surfaces UNSUPPORTED_SOURCE instead.  Sub-paths belong in the
        # separate ``path`` field.
        plugin = MarketplacePlugin(
            name="p",
            source={"type": "github", "repo": "acme/forge/extra"},
        )
        assert _resolve_plugin_github_coords(plugin, "github.com") is None
...
class TestFetchPluginApmYmlErrorMessage:
    ...
    def test_network_error_message_has_no_marketplace_update_hint(self):
        ...
        with patch(
            "apm_cli.marketplace.audit.fetch_raw",
...
class TestRunAudit:
    def test_mixed_marketplace(self):
        ...
        reports = run_audit(manifest, _source(), _fetcher=fetcher)
...
class TestAuditCLI:
    ...
    def _invoke(self, runner, extra_args=()):
        ...
        return runner.invoke(marketplace, ["audit", "mymarket", *extra_args])

    def _stub_registry_and_manifest(self, manifest, source):
        # Patch at the import site (apm_cli.commands.marketplace.audit) so the
        # module-level `from ...marketplace.X import Y` bindings are replaced.
        patches = [
            patch(
                "apm_cli.commands.marketplace.audit.get_marketplace_by_name",
                return_value=source,
            ),
            patch(
                "apm_cli.commands.marketplace.audit.fetch_marketplace",
    ...
    def test_strict_exits_nonzero_on_unverifiable_plugin(self, runner):
        # PR contract: --strict exits non-zero on bypass warnings *or*
        # unverifiable plugins (NETWORK_ERROR / PARSE_ERROR).  This guards
        # the second half of that contract, complementing the bypass test
        # above.
        source = _source("mymarket")
        manifest = _manifest(_gh_plugin("offline-plugin"))
        ...
        try:
            with patch(
                ...
                return_value=(
                    ...
                    None,
                    "fetching offline-plugin: DNS failure",
                ),
            ):
                result = self._invoke(runner, extra_args=("--strict",))
        finally:
    ...
    def test_strict_exits_nonzero_when_a_clean_plugin_is_skipped(self, runner):
        ...
        assert "--strict: 1 plugin source(s) skipped" in result.output
        assert "Run 'apm marketplace audit mymarket --strict --verbose'" in result.output
    ...
    def test_unverifiable_without_strict_exits_zero(self, runner):
        ...
        # output but the command still exits 0.
        source = _source("mymarket")
        manifest = _manifest(_gh_plugin("offline-plugin"))
    ...
    def test_clean_marketplace_exits_zero(self, runner):
        ...
        # Default mode hides the "Audit Results:" header on an all-clean run
        # so the summary is not preceded by an empty section.
        assert "Audit Results:" not in result.output

    def test_verbose_shows_clean_and_skipped_detail(self, runner):
        # Verbose surfaces per-plugin context that the default mode keeps
        # silent: clean plugins get a "marketplace-resolved" success line,
        # and skipped plugins get their skip reason inline with the status
        # code.  Without this coverage, regressions in those branches would
        # only surface when a user runs the command interactively.
        source = _source("mymarket")
        manifest = _manifest(_gh_plugin("clean"), _gh_plugin("missing-yml"))

        def fetcher(plugin, _src, _auth=None):
            if plugin.name == "clean":
                return (
                    FetchStatus.OK,
                    {"dependencies": {"apm": ["x@mkt"]}},
        ...
        try:
            with patch(
                ...
                result = self._invoke(runner, extra_args=("-v",))
        finally:
            self._stop_patches(patches)

        assert result.exit_code == 0, result.output
        # Clean plugin's verbose-only success line
        assert "clean: deps are marketplace-resolved" in result.output
        # Skipped plugin's verbose-only detail with status code
        assert "missing-yml: skipped (no_manifest" in result.output
