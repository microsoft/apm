...
def _strip_ansi(text: str) -> str:
    """Strip ANSI escape sequences emitted by Rich before content checks."""
    return _ANSI_ESCAPE.sub("", text)


def _ascii_only(text: str) -> bool:
    """Return True iff every visible codepoint is printable ASCII (U+0020-U+007E).

    ANSI escape sequences (from Rich) are stripped first since they are
    terminal control codes, not content authored by APM source strings.
    """
