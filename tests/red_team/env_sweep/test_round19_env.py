...
def test_r19_dsn_password_masked(tmp_path, monkeypatch):
    """A libpq/JDBC ``password=`` DSN value is masked in the log (control)."""
    monkeypatch.setenv("APM_HOME", str(tmp_path))
    se._append_to_script_log(
        "post-install",
        "command",
        "migrate",
        stdout=f"host=db user=app password={_OPAQUE} dbname=app",
        status="ok",
    )
    content = (tmp_path / "logs" / "scripts.log").read_text()
    assert _OPAQUE not in content, content


def test_r19_embedded_url_userinfo_masked(tmp_path, monkeypatch):
    """A tokenized git URL userinfo is masked in the log (control)."""
    monkeypatch.setenv("APM_HOME", str(tmp_path))
    se._append_to_script_log(
        "post-install",
        "command",
        "git-clone",
        stdout=f"cloning https://ci-bot:{_OPAQUE}@github.com/org/repo",
        status="ok",
    )
    content = (tmp_path / "logs" / "scripts.log").read_text()
    assert _OPAQUE not in content, content
...
def test_r19_bundler_pair_password_masked(tmp_path, monkeypatch):
    """Bundler user:password pair masked in log (control)."""
    monkeypatch.setenv("APM_HOME", str(tmp_path))
    se._append_to_script_log(
        "post-install",
        "command",
        "bundle",
        stdout=f"BUNDLE_GEMS__CONTRIBSYS__COM=svc:{_OPAQUE}",
        status="ok",
    )
    content = (tmp_path / "logs" / "scripts.log").read_text()
    assert _OPAQUE not in content, content
