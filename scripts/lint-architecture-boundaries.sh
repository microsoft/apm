...
# #2398: the entry-filtering policy used while clearing or replacing indexed
...
git_auth_config_consumers="
src/apm_cli/core/auth.py
src/apm_cli/utils/github_host.py
"
git_auth_config_duplicate_hits=$(
    grep -nE \
        '"extraheader"[[:space:]]+in[[:space:]]+|\.startswith\(\(?[[:space:]]*"(authorization:|proxy-authorization:)' \
        $git_auth_config_consumers \
        | grep -v 'architecture-authority-exempt:' \
        || true
)
if ! grep -q '^def retain_non_auth_git_config_entries(' "$git_auth_config_owner" \
    ...
    echo "[x] Git auth-config entry filtering must be owned by utils/git_env.py"
