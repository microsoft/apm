...
## 2. `devcontainer-feature` Directory Structure
...
Related repo-root configuration:

```
.devcontainer/
└── devcontainer.json                   # Points at ../devcontainer-feature/src/apm for local dev
```
...
8. **Adds `bash` on Alpine** — required because devcontainer-feature test scripts use `#!/bin/bash`.
...
## 4. How to use `devcontainer-feature` in your project

### Option A — consume it locally (this repo's own dev container)

[.devcontainer/devcontainer.json](../.devcontainer/devcontainer.json) shows the pattern:
...
```json
{
  ...
  "features": {
    "../devcontainer-feature/src/apm": {}
  }
}
```

A relative path pointing at the folder containing `devcontainer-feature.json` is enough for local development.
...
```sh
cd devcontainer-feature/test/apm/unit
