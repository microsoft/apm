...
wraps each one in Claude's required `matcher` and `hooks` group. The generated
`matcher: "*"` preserves the flat entry's global scope by matching all tools.

**Portable source (all targets):**
...
**Claude output (generated by APM):**
