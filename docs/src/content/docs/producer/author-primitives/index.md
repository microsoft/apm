...
Encode primitive Markdown as UTF-8. When a Markdown primitive has frontmatter, its opening fence of at least three hyphens (for example, `---`) must be the first content on line 1; an optional UTF-8 BOM may precede it and is stripped before parsing. Without that opening fence, APM treats the document as body content and later `---` lines remain Markdown horizontal rules. Invalid or unsafe instruction frontmatter stops the package before deployment; see [`apm install`](../../reference/cli/install/#behavior).
:::
