...
generated `.cursor/rules/<name>.mdc`. The same applies when a YAML sequence
resolves to the single universal glob. Other non-empty `applyTo` patterns
continue to compile to Cursor `globs:`; absent or empty `applyTo` emits
neither `alwaysApply` nor `globs`.
...
| cursor | `.cursor/rules/<name>.mdc` | A single universal `**` glob -> `alwaysApply: true`; other non-empty `applyTo` values -> `globs:` (scalar for single glob, YAML array for comma-lists); absent or empty `applyTo` -> neither key; description auto-derived if missing |
