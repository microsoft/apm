...
One `apm.yml`. Nine default harnesses. Reproducible AI agent setup. Every developer who clones the repo runs `apm install` and gets the same supported primitives wired into Copilot, Claude, Cursor, OpenCode, Codex, Gemini, Grok Build, Windsurf, and Kiro. Antigravity is available as an explicit CLI target. The lockfile pins exact versions and content hashes.
