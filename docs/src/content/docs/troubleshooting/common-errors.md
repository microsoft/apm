...
Fix: investigate the upstream package before accepting. If the change is legitimate, run `apm install --update` to re-pin and commit the new hash. If the lockfile was generated by a pre-fix Windows CLI, regenerate it with a current APM (`apm lock` or `apm install --update`) and commit the LF hash.
