...
`npm update`. CLI upgrades depend on how you installed APM:
[use your package manager or the standalone updater](#update-the-apm-cli-binary).
</Aside>
...
Use the tool that installed APM. For **Homebrew**:

```bash
brew upgrade apm
```

Homebrew core disables `apm self-update` and startup update notices; see the
[self-update reference](../../reference/cli/self-update/#description).
For **pip**, run `pip install --upgrade apm-cli`; for **Scoop**, run `scoop update apm`.

For **standalone installs only**, check for a release, then update through the
official installer:
...
CLI upgrades do not refresh project dependencies or change the lockfile.

<Aside type="note" title="Back-compat shim">
If you run the legacy `apm update --check`, the CLI prints a
deprecation notice and forwards to `apm self-update --check`. Use that command
directly only for standalone installs.
</Aside>
...
| Update a Homebrew-installed CLI | `brew upgrade apm` |
| Update a standalone CLI | `apm self-update` |
| Check for a standalone CLI update | `apm self-update --check` |
