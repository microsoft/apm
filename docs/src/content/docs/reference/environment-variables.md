...
| `OPENCODE_CONFIG_DIR` | Override the OpenCode user-scope configuration root used for skills, agents, commands, compiled instructions, and MCP config. | `~/.config/opencode` | Absolute values take precedence over `$XDG_CONFIG_HOME/opencode` and `~/.config/opencode`. Relative values are ignored and resolution falls through. The selected directory must already exist for automatic user-target detection; explicit `--target opencode` remains available. |
...
| `XDG_CONFIG_HOME` | Standard XDG configuration root used as the OpenCode user-scope fallback when `OPENCODE_CONFIG_DIR` is unset. | unset | Absolute values select `$XDG_CONFIG_HOME/opencode` before `~/.config/opencode`; relative values are ignored. The selected directory must already exist for automatic user-target detection. |
