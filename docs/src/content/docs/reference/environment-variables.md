...
| `ADO_APM_PAT` | Azure DevOps PAT for `ado_modules`. | unset | If unset, APM falls back to AAD bearer via `az` CLI (Azure DevOps Services only). |
| `ADO_HOST` | Single on-prem Azure DevOps Server FQDN (e.g. `ado.company.com`). Registers the host as ADO-class for auth and API routing. | unset | Takes precedence over `GITHUB_HOST` when both name the same host. Requires a valid FQDN. |
| `APM_ADO_HOSTS` | Comma-separated list of on-prem ADO Server FQDNs, all classified as ADO-class. | unset | Use when operating multiple on-prem ADO Server instances. Each entry must be a valid FQDN. |
