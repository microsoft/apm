...
| `version` | string | no | Resolved package version. For registry entries: the exact version selected from the registry for reinstall; `resolved_hash` remains the integrity anchor. For git/local entries: the `version` field from the dependency's `apm.yml` at resolution time (display/inventory metadata only -- replay always uses `resolved_ref`/`resolved_commit`). For `package_type: claude_skill`, the value is `unknown` because `SKILL.md` has no APM-owned package version. For git-semver entries: the resolved semver version. **SELF-ASSERTED** for git/local entries; see `name` boundary note above. |
