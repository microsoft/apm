...
| `results[].latest_version` | yes | Registry-selected latest version. Servers SHOULD use their normal release policy; clients treat this as display metadata. |
...
- **v1.0** (initial) — `versions`, `download`, `publish`, `search`. tar.gz + zip both supported. RFC 7807 errors. Bearer auth. Immutable versions. No yank or signing — those are v2.
