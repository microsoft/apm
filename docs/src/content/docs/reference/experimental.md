...
| `canvas`              | Ship Copilot CLI canvas extensions (`.apm/extensions/<name>/extension.mjs`) through APM packages. Dependency-provided canvases require `allowExecutables` approval (`apm approve <pkg>`). See [Canvas extensions](../integrations/canvas/). |
