...
|---|---|---|---|
| `require` | `list<string>` | `[]` | Registry names that MUST be reachable. APM **fails-closed** if a listed name has no URL in the project's `registries:` block — this is intentional: a missing registry config is treated as a policy violation, not a no-op. |
