...
| `-t, --target` | Force deployment to specific targets. Comma-separated. Values: `copilot`, `claude`, `cursor`, `opencode`, `codex`, `gemini`, `antigravity`, `windsurf`, `kiro`, `agent-skills`, `all`. `copilot-cowork` and skills-only `grok-cloud` are also accepted when their experimental flags are enabled. Experimental targets are excluded from `all`. |
