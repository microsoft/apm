...
| Executable trust | In an APM project, reports malformed `executables` configuration or local allows overridden by organization policy. Malformed configuration points to the `executables` block in `apm.yml`; policy conflicts point to `apm policy explain`. | No |
