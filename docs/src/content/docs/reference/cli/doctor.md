...
| Executable trust | In an APM project, reports malformed `executables` or deprecated `allowExecutables` configuration, naming the offending block in `apm.yml`. Local allows overridden by organization policy point to `apm policy explain`. | No |
