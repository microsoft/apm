...
- `self-update.install-dir` is expanded and stored absolute, then passed as `APM_INSTALL_DIR` when the environment variable is absent. On Unix, it must match the existing launcher directory.
...
| `self-update.install-dir` | path | unset | Installer launcher directory preference; `APM_INSTALL_DIR` overrides it. Unix updates retain the existing installation, not a new destination. |
...
5. `APM_INSTALL_DIR` overrides persisted `self-update.install-dir`; otherwise Unix updates preserve the existing launcher directory (Windows uses its installer default). Neither setting migrates a Unix install; see [ownership and migration](../../../getting-started/installation/#unix-install-ownership-and-migration).
...
For a Unix bundle already installed at `~/.local/bin`, persist self-update preferences (substitute the original launcher directory for other installs):
