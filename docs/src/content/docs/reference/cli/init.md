...
| `--target` | (prompt) | Comma-separated target list. Skips the interactive target prompt and writes targets directly. Valid values include `copilot`, `claude`, `cursor`, `opencode`, `codex`, `gemini`, `grok-build`, `antigravity`, `windsurf`, `kiro`, `agent-skills`, and `all`. |
