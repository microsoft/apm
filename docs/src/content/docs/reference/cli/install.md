...
- **Project-scope root context hint.** After `apm install`, compile-only targets such as Gemini, agents-protocol hosts, and Claude print a one-line `[i]` hint when dependency instructions require `apm compile`. Install leaves `AGENTS.md`, `CLAUDE.md`, and `GEMINI.md` unchanged until that explicit compile step.
