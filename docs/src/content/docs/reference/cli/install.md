...
- **Instruction frontmatter preflight.** APM parses instruction frontmatter only
  when an opening fence begins on line 1 after an optional UTF-8 BOM. Invalid
  YAML or unsafe decoded metadata rejects the package before any of its
  primitives are deployed. Fix the frontmatter and rerun install. `--force`
  permits hidden-character findings but never bypasses malformed YAML.
