...
- **Interrupted-install recovery.** After a successful install, APM removes inactive resolution backup directories left by interrupted earlier runs. Active install backups and entries that do not match APM's staging name format are preserved.
