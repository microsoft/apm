...
- **User-scope root context hint.** Compilation stays explicit. After `apm install -g`, when global instructions land on a root-context-only target (the AGENTS.md family, Gemini, or top-level `CLAUDE.md`), install prints a one-line `[i]` hint pointing at [`apm compile --global`](../compile/#global-compilation). No root context file is written on install. Directory-native tools (Cursor, Windsurf, Kiro, Claude rules) surface global instructions with no compile step.
