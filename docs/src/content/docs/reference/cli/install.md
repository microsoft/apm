...
| Flag | Default | Description |
|---|---|---|
| `--agent NAME` | all | Install only named agents from a dependency. Agent names are flat filename stems (for example, `reviewer` selects `reviewer.agent.md`). Repeatable and additive across installs. The sorted selection is persisted as `agents:` in `apm.yml` and `agent_subset` in `apm.lock.yaml`, so a bare reinstall or audit replay deploys the same agents. Use `--agent '*'` to reset to all agents. |
