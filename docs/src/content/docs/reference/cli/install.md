...
| `--dry-run` | off | Print the install plan without deployment writes. Positional packages appear in the preview after validation but are not added to an existing `apm.yml`; auto-bootstrap still keeps its new manifest and any explicit `--target` selection for the next run. |
