...
- **Direct registry lookup fails closed.** Registry-form MCP entries (`apm install --mcp NAME` with no `--url` and no post-`--` command) resolve one unique registry identity before writing the manifest or user config. An unreachable registry, missing identity, or ambiguous bare server name exits non-zero without changing state. When a name is absent from the built-in GitHub catalog, the error suggests an explicit retry of that exact name against the Official MCP Registry; APM never switches registries automatically, and the hint is suppressed when any registry override is active.
