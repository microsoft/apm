...
Enable the feature, declare a registry in `apm.yml`, and run install normally. APM resolves registry-sourced deps alongside git deps:

```bash
apm experimental enable registries

# apm.yml already has a registries: block and registry-routed deps
apm install
