...
| `--frozen` | off | Lockfile-only install: never write `apm.lock.yaml`, refuse to resolve anything new, and fail if `apm.yml` and `apm.lock.yaml` have drifted or if the install deploys files the lockfile does not record. Mirrors `npm ci`. Mutually exclusive with `--update`. |
