...
| `--exclude VALUE` | unset | Skip one runtime from the resolved MCP target set, whether it came from explicit `--runtime` / `--target`, manifest `targets:`, or machine discovery. |
