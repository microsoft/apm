...
- **Target contraction is reconciled.** A narrowed `targets:` in `apm.yml` is reconciled on the next non-dry-run install (not `apm lock`, which only refreshes the lockfile): deployed files, lockfile ownership, and merge-hook config/sidecar entries (`.claude/settings.json`, `.codex/hooks.json`, etc., plus their `apm-hooks.json` ownership sidecars) for the dropped target are cleaned up, even when no dependency itself changed.
