...
- **User-scope root context.** After `apm install -g`, APM compiles user-scope root context files such as `~/.claude/CLAUDE.md`, `~/.codex/AGENTS.md`, and `~/.gemini/GEMINI.md` from global instructions. Re-run with [`apm compile --global`](../compile/#global-compilation).
