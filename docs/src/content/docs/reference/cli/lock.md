...
- **Recovers from merge conflicts.** If `apm.lock.yaml` still contains git conflict markers, `apm lock` warns, discards it, and resolves from `apm.yml`; a failed run restores the discarded file. See [lockfile versioning](../../lockfile-spec/#versioning).
