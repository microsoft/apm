...
- **Target contraction is reconciled.** A successful update removes unchanged dependencies' deployed files, lockfile ownership, and merge-hook config/sidecar entries (`.claude/settings.json`, `.codex/hooks.json`, etc., plus their `apm-hooks.json` ownership sidecars) for targets no longer declared in `apm.yml`, even when no dependency ref changes.
