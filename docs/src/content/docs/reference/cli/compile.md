...
| `--clean` | Remove orphaned AGENTS.md files no longer produced by the current primitive set. For `--target claude`, also removes a stale APM-generated CLAUDE.md when deduplication suppresses CLAUDE.md generation entirely (instructions already in `.claude/rules/` and nothing else, such as a constitution or dependencies, would keep a CLAUDE.md). If a constitution or dependencies cause a CLAUDE.md to be generated, it is regenerated/overwritten rather than removed. Hand-authored CLAUDE.md files (files without the `<!-- Generated by APM CLI -->` marker) are never deleted. |
...
The dry-run output shows `[dry-run] would remove stale CLAUDE.md` when an APM-generated file would be deleted, or `[dry-run] would skip removal of CLAUDE.md: hand-authored file will not be deleted` when a hand-authored file is found. No files are written or deleted during dry-run.
...
Remove stale AGENTS.md files after deleting primitives (also removes a stale APM-generated CLAUDE.md when deduplication suppresses CLAUDE.md generation entirely):
