...
| `--clean` | Remove orphaned AGENTS.md files no longer produced by the current primitive set. For `--target claude`, also removes a stale APM-generated CLAUDE.md when `.claude/rules/` is already populated. Hand-authored CLAUDE.md files (files without the `<!-- Generated by APM CLI -->` marker) are never deleted. |
...
Remove stale AGENTS.md files after deleting primitives (also removes a stale APM-generated CLAUDE.md when `.claude/rules/` is already populated):
