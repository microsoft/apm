...
- **Shared-transitive diamond regression**: The real-CLI sequence for #2852 installs two parents sharing a dependency, verifies its instruction bytes, ownership, and installed source survive either parent's removal, then checks scoped cleanup after the last parent is removed. This guards already-correct behavior.
