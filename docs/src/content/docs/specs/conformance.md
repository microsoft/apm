...
This page is the entry point to the conformance statement APM publishes for [OpenAPM v0.1](/apm/specs/openapm-v01/). Per req-cf-002, a conformance statement MUST cite the test invocation per requirement. APM's statement is auto-generated on every commit by the spec-conformance CI gate.

## Where the statement lives

Two artifacts ship at the repository root and update on every merge:
...
Per req-cf-001, a conformance statement covers the normative statements declared in the spec; it does not certify implementation quality, performance, or operational fitness. Two statements documented as skipped (`req-mf-016`, `req-lk-018`) are recorded as known waivers with rationale in the statement itself -- the implementation deviates from the spec text on those two by an explicit, audited decision, not by silent drift.
