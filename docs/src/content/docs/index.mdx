...
APM is a dependency manager for AI agents. Declare the skills, prompts, instructions, plugins, and MCP servers your project needs in one `apm.yml`, then any developer runs `apm install` and gets the same agent context across GitHub Copilot, Claude Code, Cursor, OpenCode, Codex, Gemini, Grok Build, Windsurf, and Kiro.
...
That's it. Copilot, Claude, Cursor, OpenCode, Codex, Gemini, Grok Build, Windsurf, Kiro -- every harness configured in one command, every dependency pinned, every MCP server policy-checked before it touches disk.
