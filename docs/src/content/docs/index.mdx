...
If the Unix installer reports that `apm` is not on `PATH`, follow its
printed export command for the current shell. For a fresh ordinary-user
install with default destinations, that command is:
