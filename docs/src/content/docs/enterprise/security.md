...
In short, APM provides **dependency governance**: controlled sources, locked versions, and byte-level verification of downloaded content. It does not currently sign packages or emit SLSA-compliant provenance attestations, so treat installed packages with the same diligence you apply to any external dependency, and describe the guarantees as **APM dependency governance** rather than as supply-chain signing or attestation in compliance documentation and vendor assessments.
