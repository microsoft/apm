...
- **GitLab and plain git remotes are not auto-discovered.** GitHub, GitHub Enterprise, and Azure DevOps remotes are supported; ADO uses the org `apm` project and `_apm` repository. For unsupported remotes, pass `apm audit --ci --policy <path-or-url>` explicitly.
