...
- **GitLab and plain git remotes are not auto-discovered.** GitHub, GitHub Enterprise, and Azure DevOps remotes are supported; ADO uses the org `apm` project and `apm-policy` repository. Legacy `_apm/_apm` is tried only after a 404 from that primary location. For unsupported remotes, pass `apm audit --ci --policy <path-or-url>` explicitly.
