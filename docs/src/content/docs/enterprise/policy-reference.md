...
primary coordinate.
GitLab and plain git remotes currently fall through with no policy applied.
Repositories with no detectable git remote emit an explicit "could not
determine org" line and skip discovery.
