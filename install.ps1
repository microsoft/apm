...
try {
    ...
    # Embed an advisory REM so anyone who opens apm.cmd in an editor
    # understands the file is generated and that cmd.exe expands the
    # %LOCALAPPDATA% token at runtime; hand-edits that hard-code the
    # expanded profile path re-introduce the bug from issue #1509.
    $shimContent = "@echo off`r`nREM Generated by install.ps1 (microsoft/apm#1509). Do not hand-edit: cmd.exe expands %LOCALAPPDATA% at runtime.`r`n`"$shimTarget`" %*`r`n"
