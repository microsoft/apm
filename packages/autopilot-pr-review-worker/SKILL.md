...
description: >-
  ...
  Do NOT use for user-facing requests to triage issues, sweep a
...
It was extracted (genesis R3 EXTRACT) when a parent scheduler
...
autopilot-pr-review-worker subagent with the spawn body in
...
```
test -f ../autopilot-pr-review-worker/assets/worker-prompt.md \
  && test -f ../autopilot-pr-review-worker/assets/completion-schema.json \
  && test -f ../autopilot-pr-review-worker/scripts/owner_touch_gate.py \
  && echo "autopilot-pr-review-worker present" \
  || echo "MISSING autopilot-pr-review-worker - stop and ask the operator"
```
...
The autopilot-pr-review-worker subagent runs this loop (full detail in the spawn
body):
