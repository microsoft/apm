...
Every push from a autopilot-pr-review-worker or fix subagent MUST be followed
...
Proceed to the next step in the autopilot-pr-review-worker loop (Copilot
...
1. Record the failing job name, the run-id URL, and a 30-line
   excerpt of the failing log in the autopilot-pr-review-worker scratch
   context.
...
**Hard cap: 3 CI fix iterations per autopilot-pr-review-worker run.** Beyond
...
The autopilot-pr-review-worker records in its return:
