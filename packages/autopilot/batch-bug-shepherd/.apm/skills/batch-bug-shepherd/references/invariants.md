...
- **Drive, do not split shepherd from complete.** When a PR exists
  ...
  drive-to-merge loop -- Copilot classification, autopilot-pr-review-worker,
...
- **Single-writer interlock per artifact.** Inside the drive loop the
  autopilot-pr-review-worker posts exactly ONE comment (its own contract) and
...
- **Two-comment-per-PR cap.** Across the entire saga, a single PR
  ...
  when Phase 5 resolved a conflict). The in-loop autopilot-pr-review-worker
