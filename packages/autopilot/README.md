...
| pr | triage | yes | yes |
| pr | review | yes | yes (advisory; `panel-mode` full/lean/delta) |
| pr | merge | no | yes (summon by name; plan first; `agent-merge` for CI when available) |

Shared: `autopilot-comment` is the sole GitHub comment writer.
Workers that post load it in-session and pass `debug` plus
`source_skill`. Schedulers never comment.
...
- Each canonical skill emits an Enter card before work and an Exit
  ...
  Triage workers default `json: off` and `debug: off`. Omitted
  `json` or `debug` is not a missing-field stop. `json: on` is an
  internal receipt only, never posted on GitHub. `debug: off`
  posts the HTML receipt plus the suggested public comment.
  `debug: on` posts the full internal template prefixed with
  `[i] Skill debug is on.` `autopilot-comment` assembles that
  body and appends `Generated by <source_skill>. This comment is AI-generated and may contain errors.`
...
Advises one already-selected issue. One public comment (suggested
reply unless `debug: on`) plus `triage/recommended` and optional
...
Selects open PRs (named list or `queue-open` sweep), including
bot-authored PRs. Sweep fetch excludes `triage/recommended` and
`status/triaged`. Classifies incoming PRs, including those with no
linked issue. Never merges, never runs review or merge workers.

### `autopilot-pr-triage-worker`

Classifies one PR. One public comment (suggested reply unless
`debug: on`) plus processing/classification. Notes CODEOWNERS
owners; does not change review requests. `ready-for-review` is not
merge approval and is not a request to run PR review.
...
`autopilot-maintainer-canvas` is the Copilot App operator UI. Live
GitHub only. CODEOWNERS may accept, defer, or apply `panel-review`,
then spawn the canonical isolated schedulers and workers. The canvas
never assigns, never requests reviewers, never merges, and never
runs skill logic itself. Merge-worker spawn needs an extra confirm.
Agentic Workflows never open the canvas. CLI and Cloud still invoke
skills by name.
