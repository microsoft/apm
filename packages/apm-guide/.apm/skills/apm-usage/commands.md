...
| Command | Purpose | Key flags |
|---------|---------|-----------|
| `apm install [PKGS...]` | Install APM, MCP, and LSP dependencies for the effective target set. The target decision comes from `--target`, `apm.yml`, config defaults, or auto-detection; `--dry-run` previews without changing state and `-g/--global` writes user-scope runtimes only. Microsoft 365 Copilot Cowork is GA: use `apm install --target copilot-cowork --global` to deploy Cowork skills. | `--update`, `--refresh`, `--force`, `--frozen`, `--dry-run`, `--verbose`, `--only [apm\|mcp]`, `--target`, `--dev`, `-g`, `--trust-transitive-mcp`, `--skill NAME`, `--mcp NAME`, `--root DIR`, `--transport`, `--url`, `--env KEY=VAL`, `--header KEY=VAL`, `--mcp-version`, `--registry URL` |
...
| `apm config set KEY VALUE` | Set a config value (`auto-integrate`, `target`, `self-update.channel`, `self-update.install-dir`, `temp-dir`, `allow-protocol-fallback`, `prefer-ssh`, `copilot-cowork-skills-dir`, `mcp-registry-url`) | -- |
...
`apm config set copilot-cowork-skills-dir <absolute-path>` persists the Cowork skills directory across shells. `copilot-cowork` is generally available -- no experimental flag is required to get, set, or unset this key, and it always appears in `apm config` and bare `apm config get` output.
