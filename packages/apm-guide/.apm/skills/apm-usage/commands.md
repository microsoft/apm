...
| `apm mcp export --runtime RUNTIME` | Regenerate runtime MCP config from resolved state (repeatable `--runtime`; `--target`/`-t` aliases match install target wording; read-only, no dep resolution, no lockfile write) | `--runtime RUNTIME` / `--target RUNTIME` / `-t RUNTIME` (repeatable), `--verbose` |
