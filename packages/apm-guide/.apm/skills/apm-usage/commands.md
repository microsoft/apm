...
| `apm prune` | Remove installed packages absent from the manifest and lockfile-resolved graph; reconcile stale deployment owners without deleting files based only on ghost metadata | `--dry-run` previews package removal and ownership repair without mutation |
