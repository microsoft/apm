...
When the selected standalone binary is incompatible with the host, `apm self-update` falls back to pip when Python is available. The fallback upgrades an existing `apm-cli` installation to the release selected by the version check.
