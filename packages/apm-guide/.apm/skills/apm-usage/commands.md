...
When `apm install -g` sees a GitHub Copilot desktop App database at `~/.copilot/data.db`, it includes the `copilot-app` target automatically so workflow-shaped prompt files deploy to the App's Workflows table. Existing unmanaged workflow rows are skipped unless `--force` is used.
...
GitHub Copilot desktop App workflow deployment is available when `~/.copilot/data.db` exists (or `APM_COPILOT_APP_DB=<abs-path>` points at a valid database). Prompts that carry workflow frontmatter -- any flat top-level key of `interval`, `schedule_hour`, `schedule_day` -- can be deployed to the App's SQLite store with `apm install --target copilot-app` (project scope), `--target copilot-app --global` (user scope), or plain `apm install -g` when the App database is present. A `.prompt.md` belongs to exactly ONE surface: workflow-shape prompts go to the App DB, plain prompts go to slash-command targets. Rows always start `enabled = 0` -- you opt in from the App. Existing unmanaged workflow rows are skipped unless `--force` is used. `apm install / update / uninstall` preserve user state (`enabled`, `last_run_at`, schedule overrides). Workflows are scoped to a real Copilot App project: when the App is running APM registers the project over the App's loopback WebSocket so the project is immediately known to the webview; when the App is closed APM falls back to a direct-SQLite `BEGIN IMMEDIATE` resolver. The first install in a brand-new repo prints a one-time "restart the Copilot App once" hint (see github/github-app#5483); subsequent installs are silent. `--global` installs that carry workflow-shape prompts warn-and-proceed because workflows run with `CWD=~/.copilot` rather than a repo -- attach the row to a project from the App's Workflows tab to fix.
