...
| `apm uninstall PKGS...` | Remove packages atomically; accepts `owner/repo`, `name@marketplace`, exact declared local paths, or portable `_local/<name>` keys copied from `apm deps list`. A missing or ambiguous identifier exits nonzero before scripts or writes. | `--dry-run`, `-g` global |
...
| `apm deps list` | List manifest- and lockfile-resolved packages; ignore parent-owned embedded manifests. Local packages use actionable `_local/<name>` keys without absolute paths. | `-g` global, `--all` both scopes, `--insecure` |
