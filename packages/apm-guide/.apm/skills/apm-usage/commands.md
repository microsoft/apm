...
| `apm marketplace audit NAME` | Supply-chain audit: warn when plugin transitive deps bypass marketplace pinning; local marketplace string sources are read only within the registered root (traversal and symlink escapes are skipped) | `--strict` (CI exit-1 on bypass, fetch errors, or when all plugins are skipped), `-v` |
