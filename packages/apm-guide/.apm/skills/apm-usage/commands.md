...
| `apm init --discover` | Brownfield discovery: scan existing agent context files (instructions, agents, hooks, commands, styles, skills) across all tools and scopes, then print a proposed `apm.yml` with migration plan | `--write` persist the proposal and migrate convertible/misplaced files to `.apm/`, `--yes` skip confirmation, `--format [text\|yaml\|json]` output format |
