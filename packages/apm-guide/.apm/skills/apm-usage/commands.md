...
| `apm doctor` | Run environment diagnostics (git, network, auth, gh CLI, marketplace config, format coverage, duplicate names, version alignment); renders a pass/fail table and exits non-zero on any critical failure. Use as the first stop when `apm install` fails or CI and local disagree. | `-v` verbose detail per check |
...
| `apm marketplace doctor` _(deprecated -- use `apm doctor`)_ | Delegates to `apm doctor`; prints a deprecation notice. Remove from scripts. | `-v` |
