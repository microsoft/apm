...
**Self-hosted Git hosts (Gitea, Gogs):** virtual packages resolve via the host's `/{owner}/{repo}/raw/{ref}/{path}` URL with a Contents API v1/v3 fallback. GitLab nested-group repos (`group/subgroup/repo`) require the object form (`git: <full-url>`, `path: <virtual>`) -- shorthand is ambiguous on >2-segment paths.
