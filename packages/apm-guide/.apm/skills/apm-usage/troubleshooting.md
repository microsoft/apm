...
| TLS verification failed | Install your corporate CA into the OS trust store. For additive per-shell trust, set `APM_EXTRA_CA_BUNDLE=/path/to/ca-bundle.pem`; configure `GIT_SSL_CAINFO` separately for Git. Use `REQUESTS_CA_BUNDLE` only when you intend to replace Python's normal trust set. |
