...
- **Policy inheritance:** a child policy that uses `extends:` but omits `unmanaged_files:` no longer silently downgrades the parent org `action: deny`; the child block is treated as fully transparent (same semantics as allow-list `None` in merge). (#1248)
