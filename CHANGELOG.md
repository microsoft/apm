...
- `apm install --target goose` and `apm compile -t goose` add the Goose
  ...
  *recipes* at `.goose/recipes/<name>.yaml` (the native `goose run --recipe`
  unit; `title`/`description`/`instructions`, plus `settings.goose_model`
  when the agent pins a model), skills deploy to the cross-tool
  ...
  and is excluded from `--target all`.
