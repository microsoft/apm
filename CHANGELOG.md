...
- Non-skill integrators (agent, instruction, prompt, command) and the hook integrator's referenced-script copy paths now silently *adopt* byte-identical pre-existing files instead of treating them as user-authored. This breaks a permanent catch-22 where a degraded `apm.lock.yaml` (`deployed_files=[]`) blocked every subsequent install with `required-packages-deployed`, even when the files were already present and unchanged. Adopt is conservative-by-design: only fires for byte-identical matches, so format-transforming targets (cursor / claude / windsurf / gemini rules and commands, codex agents) intentionally do not adopt; symlinks are also rejected to preserve the path-containment invariant. Restores symmetry with `skill_integrator._dirs_equal`. (#1313)
