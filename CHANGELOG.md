...
- HTTP dependency support via dual opt-in: `--allow-insecure` CLI flag plus `allow_insecure: true` on the `apm.yml` entry are both required to install from `http://` git remotes. Transitive HTTP deps from new hosts require explicit `--allow-insecure-host HOSTNAME` (repeatable). Credential-helper suppression on HTTP attempts blocks token leakage to plaintext channels (`GIT_ASKPASS`, `GIT_TERMINAL_PROMPT`, `GIT_CONFIG_NOSYSTEM`, `credential.helper=`). Threat model documented in `enterprise/security.md`. Thanks @arika0093! (#700)
- `apm deps list --insecure` filters installed dependencies locked to `http://` sources and shows whether each one is direct or transitive via the new `Origin` column. Thanks @arika0093! (#700)
...
- Preserve custom git ports across protocols: non-default ports on `ssh://` and `https://` dependency URLs (e.g. Bitbucket Datacenter on SSH port 7999, self-hosted GitLab on HTTPS port 8443) are now captured as a first-class `port` field on `DependencyReference` and threaded through all clone URL builders. When the SSH clone fails, the HTTPS fallback reuses the same port instead of silently dropping it. Closes #661, #731 (#665)
- Detect port-like first path segment in SCP shorthand (`git@host:7999/path`) and raise an actionable error suggesting the `ssh://` URL form, instead of silently misparsing the port as part of the repository path. Closes #784 (#787)
...
- Fix `apm init` showing overwrite confirmation prompt three times on Windows CP950 terminals. Closes #602 (#647)
...
- `apm init` Next Steps panel now shows install/marketplace/plugin workflows instead of the dead-end `apm run start` reference; no extra files are created besides `apm.yml`. Closes #603 (#649)
