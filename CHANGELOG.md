...
- **`apm install <local-bundle-path>` deploys a previously packed bundle (directory or `.tar.gz`) without touching the network or the dependency resolver.** Detection is automatic when the positional argument resolves to an existing path; otherwise the command behaves as before. Integrity is verified against the bundle's embedded `apm.lock.yaml` (`pack.bundle_files` map of `path -> sha256`) before any file is written, and the deploy honours `--target`, `--global`, `--force`, `--dry-run`, plus a new `--as ALIAS` flag for log namespacing. Local installs are an imperative deploy: they record the written paths to the project lockfile (`local_deployed_files` / `local_deployed_file_hashes`) and never mutate `apm.yml`. Resolver-, registry-, MCP-, and policy-specific flags are rejected with a single consolidated error. `apm unpack` is now `[Deprecated]` -- it emits a warning that points at `apm install <bundle-path>`. (#1098)
- **`apm pack` now embeds `apm.lock.yaml` inside the bundle** with two new `pack:` fields: `target` (the export target the bundle was packed for) and `bundle_files` (a `bundle-relative-path -> sha256` map covering every file in the bundle). This unlocks integrity verification and target-mismatch warnings at install time. (#1098)
