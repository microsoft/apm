...
- `apm install` and marketplace fetches now forward `path=<org/repo>` to `git credential fill`, so Git Credential Manager users with `credential.useHttpPath = true` get the right account per URL without an account-picker prompt. (#1226)
