...
- Per-URL credential disambiguation: APM now forwards `path=<org/repo>` to `git credential fill` so Git Credential Manager users with `credential.useHttpPath = true` get the right account per URL without an account-picker prompt. The fix completes the multi-account story started in #630 and adds a regression-trap test that fails if `git credential fill` is invoked when `gh` CLI already returned a token. (#1226)
