...
- MCP server installation now respects the explicit `targets:` whitelist for all runtimes; previously only Codex and Claude Code were gated, and only when the singular `target:` key was used (e.g. `targets: [copilot]` no longer leaks an MCP write to `~/.codex/config.toml`). Repos without a `targets:` field retain the previous auto-detect semantics for backward compatibility. A malformed `targets:` field (conflicting `target:` + `targets:`, or `targets: []`) now fails closed -- no MCP files are written -- with a warning naming the field. (#1335)
