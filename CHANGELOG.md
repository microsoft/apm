...
- Autopilot PR-review worker emits the same card shape (`path: review`, `write` default `on`). The PR-review scheduler never comments, labels, assigns, or requests reviewers; `autopilot-pr-review-panel` owns the actor-session `@me` reviewer request and the one recommendation comment. Composed review still never requests the implementer.
