...
- **Cursor users can install APM package slash commands with one flag:** `apm install --allow-cursor-commands`. Package `.prompt.md` files are deployed to `.cursor/commands/*.md` when a `.cursor/` directory is present, registering them as Cursor slash commands. The shared command transformer keeps the common Claude-compatible frontmatter (`description`, `allowed-tools`, `model`, `argument-hint`, `input`); Cursor-specific keys (`author`, `mcp`, `parameters`, ...) are dropped and reported via a `diagnostics.warn()` per file so authors see the lossy transform at install time. Without the flag, command deployment is skipped for Cursor and a warning explains how to enable it. The flag exists because Cursor reads `.cursor/commands/*` as IDE-invokable code: deployment is treated as post-install code execution and gated on explicit consent (Threat #8), mirroring `npm --ignore-scripts` semantics. Other targets (Claude, OpenCode, Gemini) are explicitly opted out of the gate -- their command files are not auto-invoked at IDE startup -- and continue to deploy by default. Note: Cursor 1.6+ only; Cursor is de-emphasizing commands in favor of rules/skills, so monitor Cursor release notes for changes to this surface. (#1046)
