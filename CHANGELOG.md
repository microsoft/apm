...
- **`apm pack` bundle export now strips credential-bearing keys from `.mcp.json`** before embedding `mcpServers` in the bundle's `plugin.json`. Previously, credential-bearing keys and secret-shaped values were passed through verbatim; they are now removed recursively per the credential set documented in the [`apm pack` reference](https://microsoft.github.io/apm/reference/cli/pack/#credential-stripping-claude-mcpservers). If your bundle consumers rely on those keys being present in the bundled manifest, replace them with `$ENV_VAR` references and inject the values at MCP-host startup. (#1623)
### Removed
