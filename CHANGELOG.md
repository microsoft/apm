...
- **`apm install` no longer masks errors from local-bundle install with `UnboundLocalError`.** The `_manifest_snapshot` and `_snapshot_manifest_path` locals are now initialized at the top of the function's `try:` block, above the local-bundle early-exit branch. Previously, any exception inside that branch (notably bundle-integrity failures on Windows) would surface as `UnboundLocalError: cannot access local variable '_snapshot_manifest_path'` from the `except`/`finally` handlers, hiding the real cause. Local-bundle install integration test fixtures (`_make_bundle` helpers) now use `write_bytes(...)` instead of `write_text(...)` so newlines stay as `\n` on Windows and the `sha256` integrity check matches across platforms. Adds a regression test that monkeypatches `install_local_bundle` to raise and asserts the handler does not propagate `UnboundLocalError`. Restores green Windows CI for `tests/unit/install/test_install_local_bundle.py`.
