...
- **`apm compile` no longer silently drops global instructions (those without an `applyTo` pattern) from the generated `AGENTS.md` and `CLAUDE.md`.** The optimizer reported them as placed at the project root with `rel: 100%`, but the three rendering paths (distributed `AGENTS.md`, monolithic `AGENTS.md`, and `CLAUDE.md`) filtered them out before emission. Globally-scoped instructions now appear under a `## Global Instructions` section, matching the optimizer's `(global)` placement decision and complementing the `.github/copilot-instructions.md` support already added for the dedicated Copilot path in #1067. Existing `applyTo` behaviour, source attribution, build-id determinism, and the `applyTo: "**"` workaround are byte-identical to before. (closes #1072)
