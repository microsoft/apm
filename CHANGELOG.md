...
- **Cursor slash command deployment.** `apm install` now writes package `.prompt.md` files to `.cursor/commands/*.md` when a `.cursor/` directory exists in the project root, registering them as Cursor slash commands. The shared command transformer preserves the common subset (`description`, `allowed-tools`, `model`, `argument-hint`); Cursor-specific keys (`author`, `mcp`, `parameters`, ...) are skipped and reported via a `diagnostics.warn()` per file so authors see the lossy transform at install time. A consent warning surfaces once per package because deployed files become IDE-invokable. (#1046)
