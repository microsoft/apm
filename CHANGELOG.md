...
- **Cursor slash command deployment (opt-in).** `apm install --allow-executable-commands` writes package `.prompt.md` files to `.cursor/commands/*.md` when a `.cursor/` directory exists, registering them as Cursor slash commands. The flag mirrors `npm --ignore-scripts` semantics: because Cursor reads `.cursor/commands/*` as IDE-invokable code, deployment is gated on an explicit user opt-in (Threat #8). Without the flag, `apm install` skips command deployment for cursor-style targets and emits a warning naming the exact CLI flag to add. The shared command transformer preserves the common subset (`description`, `allowed-tools`, `model`, `argument-hint`); Cursor-specific keys (`author`, `mcp`, `parameters`, ...) are skipped and reported via a `diagnostics.warn()` per file so authors see the lossy transform at install time. (#1046)
