...
- Global OpenCode MCP support now writes `opencode.json` to the resolved user configuration root, configurable with `OPENCODE_CONFIG_DIR` and falling back through `$XDG_CONFIG_HOME/opencode` to `~/.config/opencode`.
