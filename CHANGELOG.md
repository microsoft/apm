...
- `apm install` now fetches `path:`-specified files from GitLab-sourced deps (SaaS or self-hosted) via git sparse/partial checkout instead of the REST API, fixing self-hosted GitLab instances that return HTTP 410 (API disabled). `GITLAB_APM_PAT` / `GITLAB_TOKEN` remain available as a thin REST API fallback. Path containment is enforced on the extracted file to reject symlink/traversal escapes; closes #1014. (#1740)
