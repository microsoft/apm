...
- MCP server installation now mirrors the `apm install` targets engine: the active target set (explicit `--target` / `targets:`, else directory detection, else `[copilot]` fallback) is the whitelist for every runtime. Previously only Codex and Claude Code were gated when `targets:` was omitted, and `targets: [copilot]` only became a hard whitelist when the singular `target:` key was used (so a stray `~/.codex/config.toml` write could still leak). A malformed `targets:` field (conflicting `target:` + `targets:`, or `targets: []`) now fails closed -- no MCP files are written -- with a warning naming the field. (#1335)
