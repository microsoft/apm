...
- Autopilot maintainer canvas (`packages/autopilot/autopilot-maintainer-canvas`) gives CODEOWNERS a Copilot App control surface over live GitHub: accept/defer/panel-review labels and isolated spawn of the canonical autopilot schedulers/workers. It does not assign, request reviewers, merge, or run skill logic in the canvas session. Spawn and label clicks show an immediate activity banner (Spawning vs Working). Active sessions lists schedulers and workers with Working / Idle status.
- `autopilot-comment` is the sole GitHub comment writer for autopilot skills. Its activation card includes `debug`; it posts the public body or the debug body and always appends `Generated by <source_skill>. This comment is AI-generated and may contain errors.`
...
- Autopilot PR review worker takes `panel-mode: full | lean | delta` (omitted or unknown is `lean`). Fast-path still selects the default roster; the orchestrator may add personas with a one-line `adhoc_reason`. Do not spawn inactive stubs. Shared brief is gathered once. Merge-worker runs `full` once (or `lean` on a tiny diff), then `delta`; terminal `delta` is a noop when head and watermark are unchanged.
- Autopilot PR merge worker plans before any fold or push: compare work-to-date with original scope, run a fresh panel, fold in-scope findings, then drive CI green. Copilot App spawn uses `kickoff_mode: plan`. Prefer the `agent-merge` skill when it is present; otherwise `gh pr checks --watch`. Never merge. Never request the implementer as reviewer.
- Issue and PR triage workers must load `autopilot-comment` before posting (`comment_via: autopilot-comment`). Missing that skill stops the write. Do not call `gh issue comment` or `gh pr comment`. Every posted comment gets `Generated by <skill>. This comment is AI-generated and may contain errors.`
- Autopilot maintainer canvas spawn buttons send a compact activation card (`action: spawn-isolated-session` plus nested `kickoff:` skill card) instead of a prose paragraph. Active sessions is a table of scheduler and worker rows; workers are indented under their scheduler. Open uses the Copilot App session URL. Archive is not on the canvas.
- Issue and PR triage GitHub comments are human prose only. Activation cards default `json: off` and `debug: off` (omitted is off, not a missing-field stop). `debug: off` posts the HTML receipt plus the suggested public comment. `debug: on` posts the full internal template prefixed with `[i] Skill debug is on.` Every posted comment ends with `Generated by <skill-name>. This comment is AI-generated and may contain errors.` `json: on` emits an internal `triage-recommendation` receipt only; never post JSON on GitHub. The Triage Panel agentic workflow passes `json: off` and `debug: off` and does not require a JSON tail. The PR Review Panel agentic workflow loads `autopilot-pr-review-scheduler` then runs `autopilot-pr-review-worker` in-thread; it does not compose the merge worker. PR triage auto-applies `status/deferred` when there is no linked `status/accepted` issue, thanks the author, and asks them to open an issue first per CONTRIBUTING.md.
...
- PR triage no longer skips bot-authored pull requests (Copilot, Dependabot, github-actions). They stay in the queue so maintainers can accept, review, and merge them. Bot-authored issues still skip until a human files or accepts work.
