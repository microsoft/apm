...
def _hand_authored_skip_message(rel: str) -> str:
    """Return the canonical guidance tail for a hand-authored CLAUDE.md skip event.

    Both the dry-run and live branches use this helper so future wording edits
    happen in exactly one place.  Callers prepend their own context prefix
    (e.g. "[dry-run] Skipped removal of <rel>: " or "Skipped removal of <rel>: ")
    before the returned string.
    """
    return (
        "hand-authored file will not be deleted."
        " To remove the duplicate context, delete or rename"
        f" the file manually, or prepend the line '{CLAUDE_HEADER}'"
        " to the top of the file to mark it as APM-managed, then"
        " re-run 'apm compile --clean' to have it removed automatically."
    )
                ...
                if det.exists:
                    if det.read_error is not None:
                        ...
                        hand_authored_warning = (
                            f"[dry-run] Skipped removal of {det.rel}: "
                            + _hand_authored_skip_message(det.rel)
                        )
            ...
            if config.clean_orphaned:
                ...
                if det.exists:
                    if det.read_error is not None:
                        ...
                        try:
                            # Defense-in-depth: confirm the path is still within the
                            # project root before mutating the filesystem -- matches
                            # the containment guard used for .claude/rules/ above.
                            ...
                            msg = f"Could not remove {det.rel}: {exc!s}"
                            all_warnings.append(msg)
                            self._log("warning", msg)
                    else:
                        msg = f"Skipped removal of {det.rel}: " + _hand_authored_skip_message(
                            det.rel
                        )
                        all_warnings.append(msg)
                        self._log("warning", msg)
