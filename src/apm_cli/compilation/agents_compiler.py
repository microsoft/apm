...
class CompilationConfig:
    ...
    # Overwrite hand-authored root context files instead of refusing. When
    # False (default), compiling into a repo whose CLAUDE.md / AGENTS.md /
    # GEMINI.md exists without the APM generated-marker aborts rather than
    # silently clobbering it. When True (--force), the file is overwritten
    # and a <file>.bak copy of the original is written first.
    force: bool = False
...
class AgentsCompiler:
    ...
    def compile(
        ...
        try:
            ...
            # A root context target (CLAUDE.md / AGENTS.md / GEMINI.md) exists
            # and was not generated by APM. Refuse the compile with actionable
            # guidance instead of the generic "Compilation failed" message, and
            # leave the file untouched (the writer raises before any mutation).
            self.errors.append(hand_authored_overwrite_message(e.paths))
            return CompilationResult(
                success=False,
                output_path="",
                content="",
    ...
    def _compile_distributed(
        ...
        if config.dry_run:
            ...
            # Report hand-authored AGENTS.md collisions the live run would hit
            # (cli.py does `pass` on dry-run success and never prints
            # result.content, so surface these through the logger too).
            for note in self._dry_run_overwrite_notes(
                distributed_result.content_map.keys(),
                config,
                protect=config.agents_md_mode != "managed_section",
            ):
                self.warnings.append(f"[dry-run] {note}")
                self._log("progress", f"[dry-run] {note}", symbol="info")
        ...
        if pending_outputs:
            ...
            try:
                # Guard against clobbering a hand-authored AGENTS.md. Skipped in
                # managed_section mode, where editing an existing hand-authored
                # file in place is the intended behavior (the APM-owned block is
                # replaced, the rest preserved). HandAuthoredOverwriteError is
                # not an OSError, so it propagates to compile()'s handler.
                CompiledOutputWriter().write_many(
                    pending_outputs,
                    force=config.force,
                    protect_hand_authored=config.agents_md_mode != "managed_section",
                )
    ...
    def _compile_claude_md(
        ...
        if config.dry_run:
            ...
            # Report hand-authored CLAUDE.md collisions the live run would hit,
            # so --dry-run does not silently plan an overwrite it would refuse.
            for note in self._dry_run_overwrite_notes(claude_result.content_map.keys(), config):
                preview_lines.append(f"  [dry-run] {note}")
                all_warnings.append(f"[dry-run] {note}")
                self._log("progress", f"[dry-run] {note}", symbol="info")
        ...
        try:
            # CLAUDE.md is always a full-file target; protect a hand-authored
            # CLAUDE.md from silent overwrite (bug: first compile clobbered it).
            verdict = writer.write_many(
                pending_outputs,
                force=config.force,
    ...
    def _compile_gemini_md(
        ...
        if config.dry_run:
            for note in self._dry_run_overwrite_notes(gemini_result.content_map.keys(), config):
                all_warnings.append(f"[dry-run] {note}")
                self._log("progress", f"[dry-run] {note}", symbol="info")
            return CompilationResult(
                success=len(all_errors) == 0,
        ...
        writer = CompiledOutputWriter()
        try:
            # GEMINI.md is a full-file stub target; protect a hand-authored
            # GEMINI.md from silent overwrite.
            writer.write_many(
    ...
    def _write_output_file_with_config(
        ...
        try:
            # Full mode overwrites the whole file, so guard a hand-authored
            # target. managed_section mode edits a hand-authored file in place
            # by design, so protection is disabled there.
    ...
    def _dry_run_overwrite_notes(
        self, paths: Iterable[Path], config: CompilationConfig, *, protect: bool = True
    ) -> list[str]:
        """Preview notes for hand-authored targets a real run would collide with.

        Returns one note per would-write path that exists without an APM
        generated-marker, so ``--dry-run`` reports the collision the live run
        would hit instead of silently planning an overwrite. Empty when
        protection is disabled (managed_section) or nothing collides.
        """
        if not protect:
            return []
        notes: list[str] = []
        for path in paths:
            if not file_is_hand_authored(path):
                continue
            rel = portable_relpath(path, self.base_dir)
            if config.force:
                notes.append(
                    f"would overwrite hand-authored {rel} "
                    f"(a {path.name}.bak backup would be written first)"
                )
            else:
                notes.append(
                    f"would refuse to overwrite hand-authored {rel} -- migrate it "
                    "into a local primitive or pass --force"
                )
        return notes
