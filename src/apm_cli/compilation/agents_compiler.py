...
_COPILOT_CAPABILITY = get_target_capability("copilot")
_VSCODE_TARGET_ALIASES = (
    _COPILOT_CAPABILITY.name,
    *(
        alias
        for alias in _COPILOT_CAPABILITY.aliases
        if alias != _COPILOT_CAPABILITY.compile_family
    ),
)
_KNOWN_TARGETS = accepted_target_values("compile") | {"minimal"}
...
class AgentsCompiler:
    ...
    def __init__(self, base_dir: str = ".", source_dir: str | None = None):
        ...
        self._protected_root_context_paths: set[Path] = set()
    ...
    def compile(
        ...
        self.warnings.clear()
        self.errors.clear()
        self._protected_root_context_paths.clear()
        self._logger = logger
    ...
    def _compile_distributed(
        ...
        protected_before = set(self._protected_root_context_paths)
        ...
        protected_agents_paths = self._protected_root_context_paths - protected_before
        protected_agents_count = len(protected_agents_paths)
        skipped_agents_count = len(distributed_result.content_map) - len(pending_outputs)
        nested_git_skips = max(0, skipped_agents_count - protected_agents_count)
        distributed_result.stats["root_context_files_protected"] = protected_agents_count
        if config.dry_run:
            distributed_result.stats["agents_files_generated"] = len(pending_outputs)
            distributed_result.stats["nested_git_placements_skipped"] = nested_git_skips
            return CompilationResult(
    ...
    def _compile_single_file(
        ...
        stats["root_context_files_protected"] = int(write_blocked)
    ...
    def _maybe_emit_copilot_root_instructions(
        ...
        if existing is not None and AGENTS_MD_GENERATED_MARKER not in existing:
            ...
            result.warnings.append(
                ...
                f"'{AGENTS_MD_GENERATED_MARKER}' to the top of the file. "
