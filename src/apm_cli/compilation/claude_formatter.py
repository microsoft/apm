...
class ClaudeFormatter:
    ...
    def generate_commands(
        self, prompt_files: builtins.list[Path], dry_run: bool = False
        ...
        generated_commands: builtins.dict[Path, str] = {}
        ...
        for prompt_path in prompt_files:
            try:
                ...
                command_name, content, parse_warnings = self._transform_prompt_to_command(
                    prompt_path
                )
                ...
                errors.append(f"Failed to transform {prompt_path.name}: {e!s}")
        ...
        if not dry_run and generated_commands:
            try:
                ...
                for command_path, content in generated_commands.items():
                    ...
                    verdict = SecurityGate.scan_text(content, str(command_path), policy=WARN_POLICY)
                ...
                errors.append(f"Failed to write commands: {e!s}")
    ...
    def _transform_prompt_to_command(
    ...
    ) -> tuple[str, str, builtins.list[str]]:
        ...
        has_arguments_placeholder = bool(re.search(r"\$ARGUMENTS|\$\d+", content))

        # Append $ARGUMENTS placeholder if not present
        if not has_arguments_placeholder:
            content = content + "\n\n$ARGUMENTS"
            warnings.append(f"Added $ARGUMENTS placeholder to {prompt_path.name}")
    ...
    def _build_command_content(
        self, frontmatter_dict: builtins.dict[str, str], content: str
    ) -> str:
    ...
    def discover_prompt_files(self) -> builtins.list[Path]:
        ...
        """
        prompt_files: builtins.list[Path] = []
...
class CommandGenerationResult:
    ...
    success: bool
    commands_generated: builtins.dict[Path, str]  # command_path -> content
    commands_dir: Path
    files_written: int
    warnings: builtins.list[str] = field(default_factory=list)
    errors: builtins.list[str] = field(default_factory=list)
...
def generate_claude_commands(
    base_dir: str = ".", prompt_files: builtins.list[Path] | None = None, dry_run: bool = False
) -> CommandGenerationResult:
