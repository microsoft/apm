...
class DistributedAgentsCompiler:
    ...
    def compile_distributed(
        ...
        try:
            ...
            for p in placements:
                ...
                if skip_instructions and self._is_placement_empty_shell(p, content):
            ...
            # generated_paths covers files that WILL be written this run.
            # suppressed_paths are paths that would have been empty shells;
            # with --clean they are added to the orphan set so stale shells
            # from prior runs are removed (marker-gated: hand-authored files
            # are never deleted).
            ...
            try:
                ...
                for placement in placements:
                    ...
                    # Note: placement.agents does not exist on PlacementResult;
                    # this block is wrapped in try/except for safety.
    ...
    def _is_placement_empty_shell(self, placement: PlacementResult, content: str) -> bool:
        """Return True when a placement's content is a header/footer-only shell with no body.

        A placement is an empty shell when:
        - The content between the trailing blank line after the header comments
          and the ``---`` footer delimiter contains no real text, AND
        - No constitution file exists in the placement's directory (constitution
          is injected at write time and would add body content).

        This check is performed on the raw content string rather than a brittle
        byte-count so it remains correct if the header size ever changes.

        Args:
            placement: The placement whose content to inspect.
            content: The string returned by ``_generate_agents_content``.

        Returns:
            bool: True if writing this placement would produce a content-free file.
        """
        # Check for constitution in the placement directory.  ConstitutionInjector
        # uses agents_path.parent as its base_dir, so we mirror that here.
        if read_constitution(placement.agents_path.parent) is not None:
            return False  # Constitution would add real body content

        # Inspect the content body.  The structure is:
        #   <header-comment-lines>
        #   <blank line>
        #   <body -- empty when skip_instructions>
        #   ---
        #   <footer lines>
        # We look for any non-blank, non-footer content between the first
        # blank line and the ``---`` separator.
        lines = content.splitlines()
        in_body = False
        for line in lines:
            if not in_body:
                # The blank line after header comments marks the start of the body
                if line == "":
                    in_body = True
                continue
            # We are in the body section
            if line == "---":
                # Reached footer with no body content found -- shell confirmed
                return True
            if line.strip():
                # Non-blank, non-footer content found -- not an empty shell
                return False
        # If we never reached the footer (malformed content), treat as non-empty
        return False
            ...
            for file_path in orphaned_files:
                ...
                try:
                    ...
                    if _AGENTS_MD_GENERATED_MARKER not in existing_content:
                        cleanup_messages.append(
                            f"  [!] Skipped {rel_path}: hand-authored file -- not removing"
                        )
                        continue
                    file_path.unlink()
