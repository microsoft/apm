...
class WindsurfClientAdapter(CopilotClientAdapter):
    ...
    config-file location differs: ``~/.codeium/windsurf/mcp_config.json``
    instead of ``~/.copilot/mcp-config.json``.
    """
    ...
    def update_config(self, config_updates):
        ...
        Creates the ``~/.codeium/windsurf/`` directory if it does not
        exist (Windsurf expects this directory to be present).
        """
    ...
    def get_current_config(self):
        """Read the current ``mcp_config.json`` contents."""
    ...
    def configure_mcp_server(
        ...
        """Configure an MCP server in Windsurf's ``mcp_config.json``.

        Delegates entirely to the parent implementation but prints a
        Windsurf-specific success message.
        """
        if not server_url:
        ...
        try:
            ...
            print(
                f"Successfully configured MCP server '{config_key}' for Windsurf"
            )
            return True
