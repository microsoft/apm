...
def copy_skill_to_target(
    ...
    for target in targets:
        ...
        shutil.copytree(source_path, skill_dir, ignore=ignore_symlinks_and_pin)
...
class SkillIntegrator(BaseIntegrator):
    ...
    def _promote_sub_skills(
        ...
        *,
        ...
        for sub_skill_path in sub_skills_dir.iterdir():
            ...
            shutil.copytree(sub_skill_path, target, dirs_exist_ok=True, ignore=ignore_symlinks_and_pin)
            promoted += 1
            deployed.append(target)
        return promoted, deployed
    ...
    def _integrate_native_skill(
        ...
        for idx, target in enumerate(targets):
            ...
            from apm_cli.security.gate import ignore_symlinks_and_pin as _ignore_symlinks

            _apm_filter = shutil.ignore_patterns(".apm")

            def _ignore_symlinks_and_apm(directory, contents):
                # Compose two ignore filters: drop symlinks + .apm-pin
                # (security gate) AND drop nested `.apm/` so consumers of
                # the bundle do not see the author's primitive layout leak
                # into the deployed skill tree.
                return list(
