...
KNOWN_TARGETS: dict[str, TargetProfile] = {
    ...
    #     title/description/instructions + a ``prompt`` (headless-ready: from a
    #     ``prompt:`` frontmatter key, else a generic default), an optional
    #     ``parameters:`` block (passed through verbatim so authors can
    #     template ``{{ key }}`` variables), and settings.goose_model when the
    #     agent pins a model.  MCP servers are
    #     NOT embedded per-recipe -- an APM agent declares no MCP servers;
    #     those live at package scope and are written to
    #     ~/.config/goose/config.yaml by GooseClientAdapter (which Goose reads
    #     globally at run time, honouring $XDG_CONFIG_HOME).
