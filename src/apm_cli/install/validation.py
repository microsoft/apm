...
def _validate_local_package(dep_ref, logger) -> bool:
    """Validate a local-path dependency.

    Checks the directory exists, is accessible, and contains recognisable
    package markers (``apm.yml``, ``SKILL.md``, or ``plugin.json``).
...
def _validate_virtual_package(dep_ref, package, auth_resolver, verbose_log, logger) -> bool:
    """Validate a virtual package using the GitHub downloader's validation method.

    Applies the proxy-registry short-circuit first, then delegates to
    :meth:`GitHubPackageDownloader.validate_virtual_package_exists`.
    """
    ...
    if verbose_log:
        verbose_log(
            f"Auth resolved: host={host}, org={org}, source={auth_ctx.source}, type={auth_ctx.token_type}"
        )
    ...
    def _warn(msg: str) -> None:
        ...
        # verbose is captured from the outer _validate_package_exists closure
        if verbose_log and msg.endswith(verbose_suffix):
...
def _try_ado_bearer_fallback(dep_ref, package, result, auth_resolver, verbose_log) -> bool:
    """Attempt an ADO az-cli bearer-token fallback after a PAT failure.

    Called when the initial ``git ls-remote`` probe returned non-zero with
    an auth-failure signal and the dep is on Azure DevOps.  Returns ``True``
    when the bearer fallback succeeds, ``False`` otherwise.
    """
    import subprocess
    ...
    try:
        ...
        try:
            ...
            ado_dl = GitHubPackageDownloader(auth_resolver=auth_resolver)
            if dep_ref.host:
                ado_dl.github_host = dep_ref.host
            bearer_url = ado_dl._build_repo_url(
                dep_ref.repo_url,
                use_ssh=False,
                dep_ref=dep_ref,
                token=None,
            ...
            # rather than merging on top of the PAT-context validate_env.
            # _build_git_env explicitly skips GIT_TOKEN for scheme="bearer"
            # and emits only the bearer-specific GIT_CONFIG_* injection.
...
def _validate_ado_ghes_package(dep_ref, package, auth_resolver, verbose_log, logger) -> bool:
    """Validate a package on ADO, GHES, or a generic non-GitHub host.

    Uses ``git ls-remote`` with per-dep auth.  For ADO, silently retries
    with an az-cli bearer token if the initial PAT probe fails.  Raises
    :class:`.AuthenticationError` on confirmed ADO auth failures so the
    caller renders an actionable diagnostic instead of a generic 404.
    """
    import os
    import subprocess
    ...
    # For generic hosts relax the env so native credential helpers work.
    # Config isolation is only enforced for insecure plaintext HTTP.
    if is_generic:
    ...
    urls_to_try = _build_validation_urls(
        dep_ref=dep_ref,
        ado_downloader=ado_downloader,
    ...
    if (
        ...
        if _try_ado_bearer_fallback(dep_ref, package, result, auth_resolver, verbose_log):
            return True

    # Per-attempt verbose logging is emitted inside the probe loop
    # (and by the bearer-fallback branch above), so the result is
    # already on screen by the time we get here.
...
def _build_validation_urls(
    *,
    ...
    """Build the ordered list of URLs to probe via ``git ls-remote``.

    Non-generic hosts (GHES/ADO) always return a single authenticated URL.
    Generic hosts apply explicit-scheme strict policy or fall back to the
    legacy SSH-first chain when the dep was written in shorthand.
    """
    if is_generic:
        ssh_url = ado_downloader._build_repo_url(dep_ref.repo_url, use_ssh=True, dep_ref=dep_ref)
        if explicit_scheme in ("http", "https"):
            return [package_url] if not allow_fallback else [package_url, ssh_url]
        if explicit_scheme == "ssh":
            return [ssh_url] if not allow_fallback else [ssh_url, package_url]
        # Shorthand: legacy SSH-first chain.
        return [ssh_url, package_url]
    if is_gitlab and explicit_scheme == "ssh":
...
def _validate_github_package(
    dep_ref, package, host, org, host_info, auth_resolver, verbose_log, logger
) -> bool:
    """Validate a GitHub.com package via the GitHub API with unauth-first fallback.

    Uses :meth:`AuthResolver.try_with_fallback` to probe the REST API.
    Returns ``True`` when the repository is accessible, ``False`` when not.
    """
    from ..deps.registry_proxy import is_enforce_only
    ...
    port = dep_ref.port
    if verbose_log:
        auth_ctx = auth_resolver.resolve(host, org=org, port=port)
        verbose_log(
            f"Auth resolved: host={host_info.display_name}, org={org}, "
            f"source={auth_ctx.source}, type={auth_ctx.token_type}"
        )
...
def _validate_fallback_github_package(package, auth_resolver, verbose_log, logger) -> bool:
    """Fallback validator used when ``DependencyReference.parse`` raises.

    Assumes the package is a plain ``owner/repo`` slug on GitHub.com.
    Applies a strict alphanumeric slug guard before probing the API so
    path-confusion sequences cannot reach either sink.
    """
    host = default_host()
    org = package.split("/")[0] if "/" in package else None
    repo_path = package  # owner/repo format
    # Defensive owner/repo guard: reject anything that isn't a strict
    # <owner>/<repo> slug so path-confusion sequences (`../`, embedded
    # slashes, control bytes) cannot reach either sink.
...
def _validate_package_exists(package, verbose=False, auth_resolver=None, logger=None, dep_ref=None):
    ...
    Dispatches to per-host-type helpers:
    - :func:`_validate_local_package` for local-path dependencies
    - :func:`_validate_virtual_package` for virtual (monorepo) packages
    - :func:`_validate_ado_ghes_package` for ADO, GHES, or generic hosts
    - :func:`_validate_github_package` for GitHub.com
    - :func:`_validate_fallback_github_package` when dep-ref parsing fails
    """
    ...
    try:
        ...
        # Virtual package validation (not a subdirectory on a non-GitHub host)
        if dep_ref.is_virtual and not virtual_subdir_repo_probe:
            return _validate_virtual_package(dep_ref, package, auth_resolver, verbose_log, logger)

        # ADO / GHES / non-github.com host validation
        if (
            virtual_subdir_repo_probe
            or dep_ref.is_azure_devops()
            or (dep_ref.host and dep_ref.host != "github.com")
        ):
            return _validate_ado_ghes_package(dep_ref, package, auth_resolver, verbose_log, logger)
        ...
        return _validate_fallback_github_package(package, auth_resolver, verbose_log, logger)
