...
def _validate_virtual_package_exists(
    ...
    from apm_cli.deps.registry_proxy import is_enforce_only
...
def _validate_package_exists(package, verbose=False, auth_resolver=None, logger=None, dep_ref=None):
    ...
    try:
        ...
        # Parse fallback is GitHub API-only. Reject anything that isn't a strict
        # <owner>/<repo> slug so path-confusion sequences cannot reach the API
        # or credential lookup sinks.
