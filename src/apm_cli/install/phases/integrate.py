...
# When such a lockfile is detected, _resolve_download_strategy forces
# a one-time re-download of branch-ref deps that match via content-hash
# fallback only. See PR #1158.
...
def _is_buggy_lockfile_apm_version(existing_lockfile: Any) -> bool:
    """Return True when *existing_lockfile* was generated by an APM version
    known to suffer from the branch-ref cache-drift bug (PR #1158).

    Returns False when:
    - existing_lockfile is None or has no apm_version set
    - the recorded apm_version is not in the buggy-version set

    The conservative default (False) means the self-heal path is opt-in:
    only trigger for lockfiles we can positively identify as affected.
    """
...
def _resolve_download_strategy(
    ...
    # Bug fix: branch-ref drift detection.
    #
    # When a dep uses a branch ref (e.g. ``ref: main``) and upstream has
    # advanced past the lockfile-recorded SHA, ``apm install`` (no flag)
    # MUST re-download. The previous logic short-circuited via
    # ``lockfile_match`` because local content still matched the lockfile's
    # recorded content_hash, producing a 3-way inconsistency in the
    # resulting lockfile (resolved_commit advanced, content_hash stale,
    # disk content stale). See L55 comment ("Branches always fetch latest")
    # which documented the intent but was not enforced.
    #
    # ``ref_changed`` must also be set so ``build_download_ref`` does NOT
    # pin the fresh download to the OLD locked SHA -- otherwise the fresh
    # path would re-fetch the same stale content.
    remote_drifted = (
        lockfile_match
        and not update_refs
        and resolved_ref is not None
        and resolved_ref.ref_type == GitReferenceType.BRANCH
        and _dep_locked_chk is not None
        and _dep_locked_chk.resolved_commit not in (None, "", "cached")
        and resolved_ref.resolved_commit != _dep_locked_chk.resolved_commit
    )
    if remote_drifted:
        lockfile_match = False
        ref_changed = True
        ...
        # lockfile-recorded SHA), not a supply-chain attack. Without this
        # the supply-chain hard-block at sources.py would abort the
        # install with sys.exit(1) before the repaired lockfile is written.
        ctx.expected_hash_change_deps.add(dep_ref.get_unique_key())
        if logger:
            logger.verbose_detail(
                f"  Branch ref drift: {dep_ref.get_unique_key()} remote @"
                f"{resolved_ref.resolved_commit[:8]} != locked @"
                f"{_dep_locked_chk.resolved_commit[:8]} -- forcing re-download"
            )

    # Self-heal for v<=0.12.2 branch-ref cache drift (PR #1158).
    #
    # Lockfiles generated by APM <= 0.12.2 may contain a phantom
    # ``resolved_commit`` (the remote HEAD at the time of the buggy
    # install) while the on-disk content reflects an older commit. The
    # content-hash fallback above happily declares ``lockfile_match=True``
    # because both lockfile content_hash and disk content reflect the
    # OLD bytes, so the lie persists indefinitely until upstream advances
    # again.
    #
    # When we detect this combination (buggy lockfile generator + branch
    # ref + content-hash-only match), force a one-time re-download. The
    # subsequent install records the current apm_version, so the trigger
    # does not re-fire on the next install.
    if (
        lockfile_match
        and lockfile_match_via_content_hash_only
        and not update_refs
        and resolved_ref is not None
        and resolved_ref.ref_type == GitReferenceType.BRANCH
        and _is_buggy_lockfile_apm_version(existing_lockfile)
    ):
        lockfile_match = False
        ref_changed = True
        # In the corrupted state, lockfile.content_hash
        # records OLD bytes while disk content is also OLD. The forced
        # re-download will produce NEW bytes whose hash differs from the
        # OLD lockfile hash. Tell the FreshDependencySource that this
        # change is LEGITIMATE -- otherwise the supply-chain hard-block
        # would call sys.exit(1) before we can repair the lockfile.
        ctx.expected_hash_change_deps.add(dep_ref.get_unique_key())
        _heal_msg = (
            f"Recovering {dep_ref.get_unique_key()} from "
        ...
        diagnostics.warn(_heal_msg, package=dep_ref.get_unique_key())
        if logger:
            logger.progress(_heal_msg)
