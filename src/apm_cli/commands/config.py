...
_REGISTRY_KEY_RE = re.compile(r"^registry\.([a-zA-Z0-9._-]+)\.(url|token)$")
...
def set(key, value):  # noqa: F811
    ...
    if registry_match:
        ...
        from ..config import set_registry_token, set_registry_url
...
def unset(key):
    ...
    if registry_match:
        ...
        if not is_enabled("registries"):
            ...
            sys.exit(1)
        reg_name, field = registry_match.group(1), registry_match.group(2)
        from ..config import unset_registry_token, unset_registry_url
