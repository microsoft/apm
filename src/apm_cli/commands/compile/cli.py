            ...
            if intermediate_result.success:
                ...
                if not dry_run:
                    ...
                    if c_status in ("CREATED", "UPDATED", "MISSING"):
                        ...
                        from ...compilation.output_writer import (
                            CompiledOutputWriter,
                            HandAuthoredOverwriteError,
                            hand_authored_overwrite_message,
                        )
                        ...
                        try:
                            ...
                            if config.agents_md_mode == "managed_section":
                                ...
                                # Protect a hand-authored AGENTS.md from silent
                                # overwrite; --force overwrites (with a .bak).
                        ...
                        except HandAuthoredOverwriteError as e:
                            logger.error(hand_authored_overwrite_message(e.paths))
                            sys.exit(1)
...
@click.option(
    ...
    default=False,
    help=(
        "Overwrite a hand-authored root context file (CLAUDE.md, AGENTS.md, "
        "GEMINI.md) that was not generated by APM. Without --force, compile "
        "refuses to clobber such a file and aborts. When --force overwrites an "
        "unmarked file, a <file>.bak copy of the original is written first."
    ),
)
@click.option(
    "--legacy-skill-paths",
