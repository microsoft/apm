...
def _run_compile_once(
    *,
    ...
    were silently ignored under --watch, regressing the #1019/#1074 fixes.
    Routing both paths through this function makes structural drift
    impossible.

    apm.yml is read fresh on every call so watch-mode reflects mid-session
    edits to `targets:`.

    Returns the CompilationResult.  Caller inspects ``result.errors`` and
    ``result.has_critical_security`` to decide on exit code; this function
    never calls ``sys.exit`` so the watcher can keep running after a
    failed recompile.
    """
            ...
            if intermediate_result.success:
                ...
                if not dry_run:
                    ...
                    if c_status in ("CREATED", "UPDATED", "MISSING"):
                        ...
                        # OSError propagates: one-shot caller's outer
                        # except converts to sys.exit(1); watcher's
                        # per-recompile except logs and keeps watching.
    ...
    # `result.has_critical_security` is the authoritative signal --
    # single-file mode may have flipped it in-place above when its
    # post-injection security scan found critical findings.  Caller
    # (one-shot: sys.exit 1; watcher: log & keep watching) reads it
    # directly off the returned result.
    return result
    ...
    try:
        ...
        if watch:
            if clean:
                ...
                # every recompile would surprise users mid-session, so
                # the watcher hard-codes clean=False.  Surface that
                # decision instead of silently dropping the flag.
                logger.warning(
                    "--clean is ignored in watch mode; "
                    "run 'apm compile --clean' separately to remove orphaned outputs."
                )
            _watch_mode(
                target=target,
                output=output,
        ...
        # One-shot compile: share the body with the watch path (#1345) so
        # target resolution can't drift between the two surfaces.
        result = _run_compile_once(
            target=target,
            output=output,
            chatmode=chatmode,
            no_links=no_links,
        ...
        if result.has_critical_security:
            logger.error(
