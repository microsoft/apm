...
@click.option(
    ...
    default=None,
    help="Target harness(es) to deploy to. Comma-separated for multiple: --target claude,cursor. Repeating the flag (e.g. '-t a -t b') is NOT supported -- only the last value wins; use commas. Highest-priority entry in the resolution chain (--target > apm.yml targets: > auto-detect). Values: copilot, claude, cursor, opencode, codex, gemini, windsurf, agent-skills, copilot-app, all. 'agent-skills' deploys to .agents/skills/ (cross-client). 'copilot-app' deploys scheduled prompt workflows to the GitHub Copilot desktop App when ~/.copilot/data.db exists. 'all' = copilot+claude+cursor+opencode+codex+gemini+windsurf (excludes agent-skills and copilot-app); combine explicit targets for both. 'copilot-cowork' is also accepted when the copilot-cowork experimental flag is enabled (run 'apm experimental enable copilot-cowork'). Note: '--target all' on 'apm compile' is deprecated; use 'apm compile --all' instead.",
)
@click.option(
    "--allow-insecure",
    "allow_insecure",
