...
@mcp.command(
    name="export",
    help=(
        "Regenerate runtime MCP config from resolved state.\n\n"
        "Reads apm.yml and the current lockfile to (re)write runtime-specific MCP\n"
        "configuration files (e.g. .vscode/mcp.json, .github/copilot-mcp.json)\n"
        "without re-resolving or installing dependencies.\n\n"
        "Examples:\n\n"
...
@click.option(
    ...
    required=True,
    metavar="RUNTIME",
    help="Target runtime (repeatable). E.g. vscode, copilot, codex, cursor.",
)
@click.option("--verbose", "-v", is_flag=True, help="Show detailed output")
@click.pass_context
def export(ctx, runtimes, verbose):
