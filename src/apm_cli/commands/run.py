...
def _show_available_scripts(scripts, logger) -> None:
    """Display available scripts using rich table or plain text fallback."""
...
def _show_preview_panels(
    command, compiled_command, compiled_prompt_files, script_name, logger
) -> None:
    """Display preview panels using rich or plain-text fallback."""
    try:
    ...
    try:
        ...
        if not script_name:
            ...
            if not script_name:
                ...
                _show_available_scripts(scripts, logger)
                sys.exit(1)
