...
def _render_empty_scripts(logger) -> None:
...
def _render_plain_scripts(scripts, default_script, logger) -> None:
...
def _render_rich_scripts(scripts, default_script, console) -> None:
...
def list(ctx):
    ...
    try:
        ...
        if not scripts:
            _render_empty_scripts(logger)
            return
        default_script = "start" if "start" in scripts else None
        console = _get_console()
        if console:
            try:
                _render_rich_scripts(scripts, default_script, console)
                return
            except Exception:
                pass
        _render_plain_scripts(scripts, default_script, logger)
