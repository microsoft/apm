...
def pack_cmd(  # noqa: PLR0913 -- Click handler, one param per CLI option
    ...
    if check_versions or check_clean:
        (
            ...
            ctx, options, check_versions, check_clean, json_output, logger, project_root
        )
        if not version_gate_failed and not drift_gate_failed and not gate_errors:
            # _run_release_gates may have called ctx.exit() on schema errors
            pass
