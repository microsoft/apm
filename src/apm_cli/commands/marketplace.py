...
@marketplace.command(
    help="Diagnose marketplace health issues that need network access"
)
@click.argument("name", required=True)
@click.option(
    "--strict",
...
@click.option("--verbose", "-v", is_flag=True, help="Show detailed output")
def doctor(name, strict, verbose):
    """Run network-backed checks against a registered marketplace.
    ...
    try:
        from ..marketplace.client import fetch_marketplace
        from ..marketplace.doctor import FetchStatus, run_doctor
        from ..marketplace.registry import get_marketplace_by_name

        source = get_marketplace_by_name(name)
        logger.start(f"Diagnosing marketplace '{name}'...", symbol="gear")
        ...
        reports = run_doctor(manifest, source)
        ...
        logger.error(f"Failed to diagnose marketplace: {e}")
        sys.exit(1)
