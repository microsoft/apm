"""Marketplace audit: supply-chain checks for a registered marketplace.

Resolves each plugin's own ``apm.yml`` at its pinned ref and flags
``dependencies.apm`` entries that would be resolved outside the
marketplace catalogue.  Such transitive deps would resolve via git
clone and track HEAD -- **bypassing the marketplace's version
pinning** -- which defeats the supply-chain guarantee the marketplace
is supposed to provide for consumers.

This module contains only pure logic and an opt-in network fetcher; the CLI
wiring lives in :mod:`apm_cli.commands.marketplace.audit`.
"""
...
def classify_dependency(dep: str) -> DepClassification:
    ...
    hits a git remote directly, bypassing the marketplace's version pinning.
    """
    ...
    try:
        ...
        # Let the normal install path surface that error; audit does not
        # relitigate grammar.
        return DepClassification.MARKETPLACE
...
def _normalize_dep_entry(entry) -> str | None:
...
def _resolve_plugin_github_coords(
    ...
    Returns ``None`` for plugin sources the audit cannot address (string
    ...
    # Require the exact ``owner/name`` shape.  Sub-paths belong in the
    # separate ``path`` field, so a value like ``owner/repo/extra`` is
    # a misconfiguration -- if we accepted it we would build a
    # malformed GitHub URL that 404s and gets silently reported as
    # NO_MANIFEST instead of UNSUPPORTED_SOURCE.
...
def run_audit(
    ...
    *,
    ...
    """Run all audit checks across every plugin of a marketplace manifest.
