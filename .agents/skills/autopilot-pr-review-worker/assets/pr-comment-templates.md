<!--
autopilot-pr-review-worker - PR-facing comment shapes.

Two blocks. The autopilot-pr-review-worker subagent renders the PR ADVISORY
...
## PR ADVISORY COMMENT block (autopilot-pr-review-worker -> PR)

Rendered by the autopilot-pr-review-panel skill when invoked from the
autopilot-pr-review-worker loop. The autopilot-pr-review-worker supplies the appended
...
the last push of this run (autopilot-pr-review-worker step X.8). The
...
## SUPERSEDE HANDOFF COMMENT block (autopilot-pr-review-worker -> original PR)
