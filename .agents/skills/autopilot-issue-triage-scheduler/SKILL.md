...
2. Build the queue with the helpers. Persist a `plan.md` table
   (number, slot, status). You are the sole table writer.
3. Fill the pool. One issue per slot. Prefer a new session, else a
   sub-agent, else run the worker in this thread for that one
   issue, then the next. Each slot reads the complete comment
