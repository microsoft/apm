...
if [ "$CURL_EXIT_CODE" -eq 0 ] && [ -n "$AUTH_HEADER_VALUE" ] &&
    ...
    echo -e "${BLUE}Credential rejected for public APM metadata (HTTP $METADATA_STATUS); retrying once anonymously.${NC}"
    fetch_release_metadata anonymous
fi

if [ "$CURL_EXIT_CODE" -ne 0 ]; then
    echo -e "${RED}Error: Release metadata network request failed (curl $CURL_EXIT_CODE).${NC}"
    echo "Check connectivity, proxy and TLS settings, then retry."
    exit 1
fi

if metadata_is_rate_limited; then
    echo -e "${RED}Error: Release metadata rate limit (HTTP $METADATA_STATUS).${NC}"
...
if [ -n "$APM_RELEASE_METADATA_URL" ] && [ -z "$LATEST_RELEASE" ]; then
    echo -e "${RED}Error: Empty release metadata from APM_RELEASE_METADATA_URL${NC}"
...
if ! echo "$LATEST_RELEASE" | grep -q '"tag_name":'; then
    ...
    echo -e "${RED}Error: Invalid release metadata; expected a tag_name field.${NC}"
